...
- Profile, global leaderboard, cosmetics, shop, friends, push, and admin are not beta launch-path backend dependencies.
...
- Start the game with at least two players and verify lobby -> countdown -> question.
...
- Interrupt the socket/network during a game and verify reconnect UI appears, then clears after reconnect and room state resumes.
- Complete the game and verify game -> results navigation with final standings.
...
- Interrupt the socket/network during a game and verify the reconnect overlay appears, then clears after reconnect.
