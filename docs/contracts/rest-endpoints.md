...
Register, login, and refresh also set the HttpOnly `qrs.rt` refresh cookie.
Native clients that cannot use the cookie jar may request a JSON refresh token
by sending `x-refresh-token-response: body`.
...
The body token is optional when the `qrs.rt` cookie is present. Cookie-backed
refresh and logout requests must include `x-csrf-protection: 1`.
...
- `GET /api/v1/rooms/join/:inviteCode`
- `POST /api/v1/rooms/:roomId/invite`

All room endpoints require `Authorization: Bearer <accessToken>` except
`GET /api/v1/rooms/join/:inviteCode`, which is public and rate limited.
...
Room invite codes generated by `POST /api/v1/rooms/:roomId/invite` are
16-character URL-safe tokens.
